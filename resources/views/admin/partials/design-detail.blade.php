            ...
            @for($i = 1; $i <= 10; $i++)
                ...
                <div class="border-l-4 {{ $correct ? 'border-green-500 bg-green-50' : ($answer ? 'border-yellow-500 bg-yellow-50' : 'border-gray-300 bg-gray-50') }} p-3">
                    <div class="flex justify-between items-start mb-2">
                        <h5 class="font-medium text-gray-900">Question {{ $i }}</h5>
    ...
    <div class="bg-gray-50 p-4 rounded-lg">
        ...
        <div class="grid grid-cols-1 gap-2 text-sm">
            ...
            <div>
                ...
                <span class="text-gray-900">{{ \Carbon\Carbon::parse($result->created_at)->format('M d, Y H:i:s') }}</span>
            </div>
            @if($result->updated_at)
            <div>
                <span class="font-medium text-gray-700">Last Updated:</span>
                <span class="text-gray-900">{{ \Carbon\Carbon::parse($result->updated_at)->format('M d, Y H:i:s') }}</span>
            </div>
            @endif
        </div>
    </div>
