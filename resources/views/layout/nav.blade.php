<nav id="main-navbar" class="bg-transparent border-b-2 border-gray-100 transition-all duration-300">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div class="flex items-center md:order-2 space-x-1 md:space-x-0 rtl:space-x-reverse">
            ...
            @auth
            <form action="{{ route('logout') }}" method="POST" class="hidden md:inline-block ml-4">
                @csrf
                <button type="submit"
            ...
            <ul class="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0">
                ...
                <li>
                    <a href="{{ route('home') }}#aboutSection"
                       class="block py-2 px-3 rounded-sm text-gray-900 hover:text-blue-700 transition-colors duration-200 md:p-0 fw-500">About Us</a>
                </li>
                <li>
                    <a href="{{ route('home') }}#contactSection"
                       class="block py-2 px-3 rounded-sm text-gray-900 hover:text-blue-700 transition-colors duration-200 md:p-0 fw-500">Contact Us</a>
                </li>

                @guest
                <li class="block md:hidden">
                    <a href="{{ route('login.form') }}"
                       class="block py-2 px-3 rounded-sm text-gray-900 hover:text-blue-700 transition-colors duration-200 fw-500">Login</a>
                </li>
                <li class="block md:hidden">
                    <a href="{{ route('register.form') }}"
                ...
                <li class="block md:hidden">
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit"
                                class="block py-2 px-3 rounded-sm text-white bg-red-600 hover:bg-red-700 transition-colors duration-200 fw-500">
                            Logout
                        </button>
                    </form>
                </li>
